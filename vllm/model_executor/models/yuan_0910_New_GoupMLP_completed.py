...
class GroupedMLP(nn.Module):
    ...
    def __init__(self, config: YuanConfig, params_dtype: Optional[torch.dtype] = None,):
        ...
        if params_dtype is None:
            params_dtype = torch.get_default_dtype()
        self.params_dtype = params_dtype
        #self.w1 = nn.ModuleList([nn.Linear(config.hidden_size, self.ffn_hidden_size * 2, bias=False) for _ in range(self.num_experts)])
        #self.w2 = nn.ModuleList([nn.Linear(self.ffn_hidden_size, config.hidden_size, bias=False) for _ in range(self.num_experts)])
        ...
        if self.use_moe:
            for layer_id in range(self.config.num_hidden_layers):
                ...
                for expert_id in range(self.config.moe_config['moe_num_experts']):
                    ...
                    name = f'model.layers.{layer_id}.mlp.experts.w1.weight'
                    param = params_dict[name]
                    weight_loader = getattr(param, 'weight_loader', default_weight_loader)
                    weight_loader(param, moe_state_dict[name])

                    #name = f'model.layers.{layer_id}.mlp.experts.w2.{expert_id}.weight'
                    name = f'model.layers.{layer_id}.mlp.experts.w2.weight'
                    param = params_dict[name]
                    weight_loader = getattr(param, 'weight_loader', default_weight_loader)
                    weight_loader(param, moe_state_dict[name])
                '''
