...
class ParallelAttention_router(nn.Module):
    def __init__(self, config, cache_config: Optional[CacheConfig] = None, quant_config: Optional[QuantizationConfig] = None):  # AutoGPTQ models with mlp.gate quant layer
        super(ParallelAttention_router, self).__init__()

        self.hidden_size = config.hidden_size
        self.projection_size = config.moe_config['moe_num_experts']
        self.query_key_value = ReplicatedLinear(self.hidden_size, self.projection_size*3, bias=False, quant_config=quant_config) # AutoGPTQ models with mlp.gate quant layer
...
class YuanMoeLayer(nn.Module):
    def __init__(self, config:YuanConfig, quant_config: Optional[QuantizationConfig] = None): # Quant Use
        ...
        self.gate = ParallelAttention_router(config, quant_config=quant_config) # AutoGPTQ Use for mlp.gate quant
